...
func GetAuthorization(w http.ResponseWriter, r *http.Request) {
	...
	var soonestRetryAfter time.Time
	for _, ch := range az.Challenges {
		if ch.Retry != nil {
			log.Printf("%v\n", *ch.Retry)
		} else {
			log.Printf("nil retry\n")
		}
		if ch.Status == acme.StatusProcessing && ch.Retry != nil && ch.Retry.Active() {
			...
			if soonestRetryAfter.IsZero() || nextAttempt.Before(soonestRetryAfter) {
				soonestRetryAfter = nextAttempt
			}
		}
	}

	if !soonestRetryAfter.IsZero() {
		if soonestRetryAfter.After(time.Now()) {
			w.Header().Add("Retry-After", soonestRetryAfter.Add(time.Second).Format(http.TimeFormat))
		} else {
...
func GetChallenge(w http.ResponseWriter, r *http.Request) {
	...
	if ch.Status == acme.StatusPending {
		...
		if err := db.UpdateChallenge(ctx, ch); err != nil {
			render.Error(w, r, acme.WrapErrorISE(err, "error updating challenge"))
			return
		}
	...
	if ch.Status == acme.StatusProcessing && ch.Retry != nil && ch.Retry.Active() {
		time.AfterFunc(retryInterval, func() {
			retryChallenge(ctx, ch.ID, azID, acc.ID, payload.value)
		})
	}
...
//
// see: GetChallenge
func retryChallenge(ctx context.Context, chID, azID string, accID string, value []byte) {
	db := acme.MustDatabaseFromContext(ctx)
	...
	if time.Now().Before(nextAttempt) {
		...
		time.AfterFunc(time.Until(nextAttempt), func() {
			retryChallenge(ctx, chID, azID, accID, value)
		})
		return
	}
	...
	// Validation was unsuccessful, but no terminal error occurred.
	if ch.Status == acme.StatusProcessing {
		// If validation fails and retries remain, update retry state and schedule next retry
		if ch.Retry != nil && ch.Retry.Active() {
			...
			// If no retries remain, mark as invalid
